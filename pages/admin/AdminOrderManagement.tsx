...
const AdminOrderManagement: React.FC = () => {
    ...
    const filteredOrders = useMemo(() => {
        ...
        return orders.filter(order =>
            ...
            order.id.toLowerCase().includes(lowercasedQuery) || // Search full ID
            `#${order.id.slice(-6)}`.toLowerCase().includes(lowercasedQuery) // Search displayed ID
        );
    }, [orders, searchQuery]);
                        ...
                        {filteredOrders.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="text-center py-10 text-gray-500">
                                     {searchQuery ? 'No orders match your search query.' : 'There are no orders in the system yet.'}
                                </td>
                            </tr>
                        ) : (
                            filteredOrders.map(order => (
                                <React.Fragment key={order.id}>
                                    <tr 
                                        className={`border-b hover:bg-gray-100 cursor-pointer transition-colors ${expandedOrderId === order.id ? 'bg-teal-50' : ''}`}
                                        ...
                                        <td className="py-3 px-4 whitespace-nowrap">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="text-red-600 p-1 rounded-md hover:bg-red-100 disabled:text-gray-400 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                                                    title={order.status !== 'Delivered' && order.status !== 'Cancelled' ? 'Only delivered or cancelled orders can be deleted' : 'Delete order'}
                                                >
                                        ...
                                        <tr className="bg-white">
                                            <td colSpan={6} className="p-4 border-b">
                                                <div className="p-4 bg-gray-50 rounded-md border border-gray-200">
                                                    <h4 className="font-bold mb-2 text-gray-800">Order Items:</h4>
