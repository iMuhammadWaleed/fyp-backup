...
const CatererLoginPage: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (result.success && result.user) {
            if (result.user.role === UserRole.CATERER) {
                const from = location.state?.from?.pathname || '/caterer';
                navigate(from, { replace: true });
            } else {
                 setError('This is the caterer portal. Other account types are not permitted.');
                 logout();
            }
        } else {
            setError(result.message || 'Invalid email or password, or not a caterer account.');
        }
    };
        ...
        <div className="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 h-full bg-cover bg-center" style={{ backgroundImage: "url('https://picsum.photos/seed/auth-bg/1600/1200')"}}>
            <div className="max-w-md w-full bg-slate-50/90 backdrop-blur-sm rounded-xl shadow-2xl overflow-hidden">
                <div className="text-center p-8 bg-slate-100/90 border-b border-slate-200">
                    <AppLogo />
                    <h2 className="mt-6 text-center text-2xl font-bold text-gray-900">Caterer Partner Sign In</h2>
                </div>
                <div className="p-8">
                        ...
                        <div className="rounded-md shadow-sm space-y-4">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder="Caterer email address"
                                />
                            </div>
                            <div>
                                <div className="relative">
